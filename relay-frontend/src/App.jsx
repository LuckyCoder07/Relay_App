...
async function fetchDefaultCatalog(payment = "") {
  try { return await fetchJSON(`${API}/products?payment=${encodeURIComponent(payment)}`); } catch { return []; }
}
async function fetchSearch(q, payment = "") {
  try { return await fetchJSON(`${API}/products?search=${encodeURIComponent(q)}&payment=${encodeURIComponent(payment)}`); } catch { return []; }
}
async function fetchByCategory(slug, payment = "") {
  try { return await fetchJSON(`${API}/products?category=${encodeURIComponent(slug)}&payment=${encodeURIComponent(payment)}`); } catch { return []; }
}
...
function rankOffers(product, preferredPayment) {
  // Use the pre-ranked array from the backend
...
export default function RelayApp() {
  ...
  // initial load: category list + user cache
  useEffect(() => {
    (async () => {
      const [cats] = await Promise.all([fetchCategoryList()]);
      setCategoryList(cats);
      
      const cachedUser = localStorage.getItem("relay:user");
      if (cachedUser) {
        try {
          const u = JSON.parse(cachedUser);
          setUser(u);
          const [myPrefs, myOrders] = await Promise.all([
            loadPrefs(),
            u.role === "Admin" ? loadAllOrders() : loadOrders()
          ]);
          setPrefs(myPrefs);
          if (u.role === "Admin") setAllOrders(myOrders); else setOrders(myOrders);
        } catch {}
      }
  ...
  async function handleLogin(email, password) {
    try {
      ...
      if (!res.ok) { pushToast(data.error || "Login failed"); return; }
      ...
      if (data.user.role === "Admin") setAllOrders(await loadAllOrders());
      else setOrders(await loadOrders());
      
      pushToast("Welcome back, " + data.user.name);
      goto(data.user.role === "Admin" ? "admin" : "catalog");
    } catch (e) { pushToast("Network error"); }
  }
  ...
  async function handleSignup(name, email, password) {
    try {
      ...
      const data = await res.json();
      if (!res.ok) { pushToast(data.error || "Signup failed"); return; }
  ...
  function handleLogout() { 
    ...
    localStorage.removeItem("relay:user"); 
    setOrders([]); 
    setPrefs({ payment: "", chat: [] }); 
    goto("landing"); 
  }

  function isInCart(id) { return cart.some((x) => x._id === id); }
  function toggleCart(product) {
    const already = isInCart(product._id);
    setCart((c) => (already ? c.filter((x) => x._id !== product._id) : [...c, product]));
    pushToast(already ? `${product.name} removed from compare cart` : `${product.name} added to compare cart`);
  }
  function removeFromCart(id) { setCart((c) => c.filter((x) => x._id !== id)); }

  async function redirectToBuy(product, offer) {
    if (user) {
      try {
        const res = await fetch(`${API}/orders/redirect`, {
          method: "POST",
          headers: { "Content-Type": "application/json", ...getAuthHeaders() },
          body: JSON.stringify({ productId: product._id, platform: offer.platform })
        });
        if (res.ok) {
    ...
    window.open(offer.url && offer.url !== "#" ? offer.url : (PLATFORMS[offer.platform]?.search(product.name) || "#"), "_blank", "noopener,noreferrer");
  }
  ...
  async function adminSaveProduct(prod) {
    const next = products.some((p) => p._id === prod._id) ? products.map((p) => (p._id === prod._id ? prod : p)) : [...products, prod];
    setProducts(next); await saveCatalogOverride(next); setHasOverride(true);
    pushToast(`Product ${prod.name} saved (admin override active)`);
  }
    ...
    <main className="rl-section">
      ...
      <div className="rl-list">
        ...
        {!loading && products.map((p, i) => {
          ...
          return (
            <div key={p._id} className="rl-card" style={{ animationDelay: Math.min(i, 8) * 50 + "ms" }}>
              ...
              <div className="rl-card-right">
                ...
                <div className="rl-card-foot">
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button className="rl-btn-ghost" style={{ padding: "0 10px", height: "30px", fontSize: "12px" }} onClick={(e) => { e.stopPropagation(); onOpen(p); }}>
                      View details
                    </button>
...
function ProductDetail({ product, allProducts, preferredPayment, onBack, onToggleCart, isInCart, onBuy, onOpen }) {
  ...
  const related = allProducts.filter((p) => p.category === product.category && p._id !== product._id);
  const more = (related.length > 0 ? related : allProducts.filter((p) => p._id !== product._id)).slice(0, 10);

  const specs = [
    { icon: Tag, label: "Brand", value: product.brand || "Not specified" },
    { icon: Box, label: "SKU", value: product.sku || "Not specified" },
    product.weight ? { icon: Package, label: "Weight", value: product.weight + "g" } : null,
    product.dimensions && product.dimensions.width ? { icon: LayoutGrid, label: "Dimensions", value: `${product.dimensions.width}×${product.dimensions.height}×${product.dimensions.depth} cm` } : null,
    { icon: ShieldCheck, label: "Warranty", value: product.warranty || "Not specified" },
    { icon: Truck, label: "Shipping", value: product.shipping || "Not specified" },
    { icon: RotateCcw, label: "Returns", value: product.returnPolicy || "Not specified" },
    { icon: Calendar, label: "Min. order qty", value: product.minOrderQty || 1 },
  ].filter(Boolean);
      ...
      <div className="rl-pd-grid">
        ...
        <div className="rl-pd-info">
          ...
          <div className="rl-verdict-box">
            ...
            <p><strong>{best.platform}</strong> is your best option at <strong>{inr(best.price)}</strong> — {best.deliveryDays === 0 ? "store pickup today" : `delivery in ${best.deliveryDays} day${best.deliveryDays > 1 ? "s" : ""}`}, {best.paymentOffer.toLowerCase()}{preferredPayment ? `. Matched against your saved ${preferredPayment}.` : "."}</p>
          </div>
            ...
            {ranked.map((o) => (
              <div key={o.platform} className="rl-offer-row">
                <div className="rl-offer-left">
                  ...
                  <div><div className="rl-offer-platform">{o.platform}</div><div className="rl-offer-meta"><Truck size={12} /> {o.deliveryDays === 0 ? "Today" : o.deliveryDays + "d"} · {o.stock}</div><div className="rl-offer-meta">{o.paymentOffer}</div></div>
                </div>
        ...
        <div className="rl-related">
          ...
          <div className="rl-related-scroll">
            {more.map((p) => {
              ...
              return (
                <button key={p._id} className="rl-related-card" onClick={() => onOpen(p)}>
    ...
    <main className="rl-section">
      ...
      <div className="rl-cart-list">
        {cart.map((p) => {
          const best = rankOffers(p, preferredPayment)[0];
          return (
            <div key={p._id} className="rl-cart-row">
              ...
              <button className="rl-icon-btn danger" onClick={() => onRemove(p._id)}><Trash2 size={15} /></button>
            </div>
          );
        })}
      </div>
...
function AdminView({ products, onSave, onDelete, allOrders, hasOverride, onReset, loading }) {
  ...
  function blank() {
    return { id: "custom-" + Date.now(), name: "", category: "home-decoration", categoryLabel: "Home Decoration", image: "", brand: "", sku: "", warranty: "1 year warranty", shipping: "Ships in 2-3 days", returnPolicy: "30 days return", minOrderQty: 1, rating: 4.0, blurb: "",
      offers: [{ platform: "Amazon", price: 0, mrp: 0, deliveryDays: 2, paymentOffer: "", stock: "In stock" }] };
  }
  return (
    <main className="rl-section">
      ...
      {tab === "products" && (
        <>
          ...
          <div className="rl-admin-table">
            {products.map((p) => (
              <div key={p._id} className="rl-admin-row">
                ...
                <button className="rl-icon-btn danger" onClick={() => onDelete(p._id)}><Trash2 size={15} /></button>
              </div>
            ))}
          </div>
        </>
      ...
      <div className="rl-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {form.offers.map((o, i) => (
          <div key={i} className="rl-offer-form-row">
            ...
            <input placeholder="Delivery days" type="number" value={o.deliveryDays || 0} onChange={(e) => setOffer(i, "deliveryDays", parseInt(e.target.value) || 0)} />
        ...
        <button className="rl-btn-ghost" onClick={() => setForm({ ...form, offers: [...form.offers, { platform: "Amazon", url: "", price: 0, mrp: 0, deliveryDays: 2, paymentOffer: "", stock: "In stock" }] })}><Plus size={14} /> Add platform offer</button>
...
function ChatPanel({ user, prefs, products, onSavePayment, onChatUpdate }) {
  ...
  function reply(text) {
    ...
    if (found) {
      ...
      return `For ${found.name}, my pick is ${best.platform} at ${inr(best.price)} (${(best.paymentOffer || "").toLowerCase()}). Open the product page to see the full comparison and buy.`;
    }
...
/* nav */
.rl-nav{ position:sticky; top:0; z-index:100; background:rgba(245,246,252,.85); backdrop-filter:blur(12px); border-bottom:1px solid var(--line); }
.rl-nav-inner{ max-width:1180px; margin:0 auto; padding:0 20px; height:62px; display:flex; align-items:center; justify-content:space-between; gap:16px; }
.rl-logo{ background:none; border:none; font-family:'Space Grotesk',sans-serif; font-size:20px; font-weight:700; cursor:pointer; color:var(--ink); display:flex; align-items:center; gap:2px; }
.rl-logo-mark{ background:linear-gradient(120deg,var(--primary),var(--primary2)); color:#fff; width:28px; height:28px; border-radius:8px; display:inline-flex; align-items:center; justify-content:center; font-size:14px; margin-right:5px; }
.rl-nav-links{ display:flex; align-items:center; gap:6px; }
.rl-nav-links button{ background:none; border:none; color:var(--muted); font-size:14px; cursor:pointer; padding:7px 12px; border-radius:8px; display:flex; align-items:center; gap:5px; transition:color .2s, background .2s; }
.rl-nav-links button:hover{ color:var(--ink); background:var(--surface-soft); }
.rl-nav-cta{ background:linear-gradient(120deg,var(--primary),var(--primary2)) !important; color:#fff !important; border-radius:9px !important; font-weight:700 !important; padding:8px 16px !important; }
.rl-nav-cta:hover{ opacity:.9; }
.rl-badge{ background:var(--primary); color:#fff; font-size:10px; font-weight:800; width:18px; height:18px; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; }
.rl-burger{ display:none; background:none; border:none; color:var(--ink); cursor:pointer; }

/* hero */
.rl-hero{ background:linear-gradient(145deg,#F5F6FC 0%,#EEF0FA 40%,#E8ECFF 100%); padding:80px 20px 70px; overflow:hidden; position:relative; }
.rl-orb{ position:absolute; border-radius:50%; filter:blur(80px); opacity:.18; pointer-events:none; }
...
.rl-hero-grid{ display:grid; grid-template-columns:1.1fr .9fr; gap:48px; align-items:center; position:relative; z-index:1; max-width:1180px; margin:0 auto; }
...
@media (max-width: 760px){
  .rl-nav-links{ position:fixed; top:62px; left:0; right:0; background:var(--surface); border-bottom:1px solid var(--line); flex-direction:column; align-items:flex-start; padding:16px 20px; gap:16px; transform:translateY(-130%); transition:transform .25s ease; z-index:99; }
